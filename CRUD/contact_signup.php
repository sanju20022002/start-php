...
if($_SERVER['REQUEST_METHOD']=='POST'){
    if(empty($_POST['name'])){
        ...
        if (!empty($email)){
            $sql="select*from contact_signup where email='$email'";
            ...
            elseif ($password != $c_password){
                echo "<script>
                alert ('Password or confirm password not matched matched' );
                </script>";  
            }else{
                $sql="insert into contact_signup (name,email,password)values('$name','$email','$password')";
...
.reg-heading{
    ...
    font-weight: 500;
    letter-spacing: 3px;
    background-color:rgb(201, 45, 82);
    padding-top:20px;
    padding-bottom: 20px;
    color: #ffffff;
...
.reg-button{
    ...
    font-weight: 400;
    font-size: 16px;
    background-color:rgb(222, 82, 126);
    color: #ffffff;
    border: none;
    letter-spacing: 2px;
}
   ...
   <div class="main-container">
        <!-- Form Section Start -->
            <form action="" method="post">
                <h2 class="reg-heading"> contact_signup</h2>
                ...
                <!-- <div class="button"> -->
                    <div class="button">
                     <button type="submit" class="reg-button">signup</button>
                    </div> 
                <!-- </div> -->
